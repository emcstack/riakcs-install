...
main ()
{
  ...
  yum_repo_config_url="https://packagecloud.io/install/repositories/basho/riak-cs/config_file.repo?os=${os}&dist=${dist}&source=script"

  yum_repo_path=/etc/yum.repos.d/basho_riak-cs.repo
  ...
  yum install -y pygpgme --disablerepo='basho_riak-cs'
  ...
  if [ "$?" != "0" ]; then
    ...
    sed -i'' 's/repo_gpgcheck=1/repo_gpgcheck=0/' /etc/yum.repos.d/basho_riak-cs.repo
  fi

  echo "Installing yum-utils..."
  yum install -y yum-utils --disablerepo='basho_riak-cs'
  ...
  echo "Generating yum cache for basho_riak-cs..."
  yum -q makecache -y --disablerepo='*' --enablerepo='basho_riak-cs'
